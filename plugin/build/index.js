...
const modifyContentSwift = (contents, urlScheme) => {
...
${NAVER_HEADER_IMPORT_STRING_SWIFT}`);
    ...
    if (!contents.includes('NaverThirdPartyLoginConnection.getSharedInstance')) {
        contents = contents.replace('return super.application(app, open: url, options: options) || RCTLinkingManager.application(app, open: url, options: options)', `if (url.scheme == "${urlScheme}") {
...
const modifyAppDelegate = (config, { urlScheme }) => {
    return (0, config_plugins_1.withAppDelegate)(config, async (config) => {
        if (['objc', 'objcpp'].includes(config.modResults.language)) {
            ...
            config.modResults.contents = modifyContentSwift(config.modResults.contents, urlScheme);
            config_plugins_1.WarningAggregator.addWarningIOS('withNaverLogin', 'Swift AppDelegate is experimental.');
        }
        return config;
    });
};
